 ...
 *
 * This version (2t) generates 2 files:
 *      -File containing 1 or more partitions where the start and goal cell share a partition
 *      -File containing only 1 partition where all cells are in the same partition
 *
...
int main (int argc, char* argv[])
{
    ...
    if (components > 1)  // output intermediate result the first time goal is reachable - after one pass of "Connect"
    {
        char outFileName1[100] = {'\0'}; //fill with \0 .. assume 100 is long enough

        //put first part of the name in the outFileName
        char a = fileName[0];
        size_t initCount = 0;
        while (a != '\0')
        {
            outFileName1[initCount] = a;
            ++initCount;
            a = fileName[initCount];
        }

        //now, append the period
        outFileName1[initCount] = '.';
        ++initCount;

        size_t calcComponents = components;

        size_t digits = 1 + (size_t)log10(calcComponents);

        //increase the count by the number of digits, since we need to start from right side
        initCount += digits - 1;

        for (size_t i = 0; i < digits; ++i)
        {
            char digit = '0' + (char)(calcComponents % 10);
            // put this digit in the correct place in filec
            outFileName1[initCount] = digit;
            --initCount; //decrease counter
            calcComponents /= 10;
        }

        //open new file
        std::ofstream out1(outFileName1, std::ofstream::out);

        // output first pass maze
        WriteMaze(out1, start, goal, numrows, numcols, maze);
        out1.close();
        std::cout << "1-pass maze written to file " << outFileName1 << '\n';
    }
    ...
    out2.close();
    std::cout << "n-pass maze written to file " << outFileName2 << '\n';
